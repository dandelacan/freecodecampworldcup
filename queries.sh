...
echo "$($PSQL   "SELECT DISTINCT name FROM teams FULL JOIN games AS winners ON teams.team_id = winners.winner_id FULL JOIN games AS opponents ON teams.team_id = opponents.opponent_id WHERE (winners.year=2014 AND winners.round='Eighth-Final') OR (opponents.year=2014 AND opponents.round='Eighth-Final') ORDER BY name")"
